...
{
    private static readonly MethodInfo _clampMethod = AccessTools.Method(
        typeof(Mathf),
        nameof(Mathf.Clamp),
    ...
    private static readonly Type _sliderTailDataType =
        Type.GetType("BeatmapObjectsInTimeRowProcessor+SliderTailData,BeatmapCore") ??
        throw new InvalidOperationException();

    private static readonly Func<IEnumerable, IEnumerable> _getSliderTailDatas =
        (Func<IEnumerable, IEnumerable>)AccessTools
            .Method(typeof(Enumerable), nameof(Enumerable.OfType))
    ...
    private static void ProcessAllNotesInTimeRowPatch(
        NoodleBeatmapObjectsInTimeRowProcessor __instance,
        int ____numberOfLines,
        object allObjectsTimeSlice)
    {
        ...
        foreach (CustomNoteData noteData in notesInTimeRow)
        {
            ...
            {
                IEnumerable<float?>? listPosition =
                    list[k].customData.GetNullableFloats(v2 ? V2_POSITION : NOTE_OFFSET);
        ...
        foreach (CustomSliderData sliderData in slidersInTimeRow)
        {
            ...
            foreach (CustomNoteData noteData in notesInTimeRow)
            {
                IEnumerable<float?>? notePosition =
                    noteData.customData.GetNullableFloats(v2 ? V2_POSITION : NOTE_OFFSET)?.ToList();
    ...
    private static void ProcessColorNotesInTimeRowPatch(
        NoodleBeatmapObjectsInTimeRowProcessor __instance,
        int ____numberOfLines,
        object currentTimeSlice)
    {
        float offset = ____numberOfLines / 2f;
